...
void Nanoshield_MRF::setCSMACAMode(bool coord)
{
  ...
  writeShort(MRF_TXMCR, reg | 0b00100000);
  if(coord)
  {
...
/*selectBuffer == 1 RXbuffer else TXbuffer*/
void* Nanoshield_MRF::getBufferData(bool selectBuffer)
{
	return(selectBuffer? &rxBuf : &txBuf);
}

void Nanoshield_MRF::setPromiscuous(bool enabled) {
    if (enabled) {
        writeShort(MRF_RXMCR, 0x01);
    } else {
